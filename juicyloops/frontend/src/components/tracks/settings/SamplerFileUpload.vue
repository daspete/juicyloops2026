...
const onFileSelect = async (event: FileUploadSelectEvent) => {
    props.track.setFile(Array.isArray(event.files) ? event.files[0] : event.files);
};
</script>
...
<template>
    <div v-if="props.track" class="flex items-center gap-1">
        <div class="flex items-center gap-1 rounded bg-surface-800">
            <FileUpload mode="basic" @select="onFileSelect" customUpload auto :chooseLabel="props.label || 'Select a sample'" :chooseButtonProps="{ size: 'small' }">
                <template #chooseicon>
