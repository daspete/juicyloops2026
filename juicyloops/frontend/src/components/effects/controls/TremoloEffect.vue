...
const wetValue = computed({
    get: () => props.track.effects.tremolo.wet.value,
    set: (value: number) => {
        props.track.effects.setTremolo({
            wet: value,
        });
    },
});
...
const depthValue = computed({
    get: () => props.track.effects.tremolo.depth.value as number,
    set: (value: number) => {
        props.track.effects.setTremolo({
            depth: value,
        });
...
const frequencyValue = computed({
    get: () => props.track.effects.tremolo.frequency.value as number,
    set: (value: number) => {
        props.track.effects.setTremolo({
            frequency: value,
        });
...
<template>
    <div class="rounded p-2 bg-surface-800 w-52 flex flex-col gap-2">
        <div class="font-semibold">Tremolo</div>
