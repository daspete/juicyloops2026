...
const wetValue = computed({
    get: () => props.track.effects.vibrato.wet.value,
    set: (value: number) => {
        props.track.effects.setVibrato({
            wet: value,
        });
    },
});
...
const depthValue = computed({
    get: () => props.track.effects.vibrato.depth.value as number,
    set: (value: number) => {
        props.track.effects.setVibrato({
            depth: value,
        });
...
const frequencyValue = computed({
    get: () => props.track.effects.vibrato.frequency.value as number,
    set: (value: number) => {
        props.track.effects.setVibrato({
            frequency: value,
        });
...
<template>
    <div class="rounded p-2 bg-surface-800 w-52 flex flex-col gap-2">
        <div class="font-semibold">Vibrato</div>
